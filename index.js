...
const { data, error } = await supabase.from('arcade').select('"User Name", "Google Cloud Skills Boost Profile URL", "# of Skill Badges Completed", "# of Arcade Games Completed", "# of Trivia Games Completed", "Access Code Redemption Status"').order([{column: '"# of Skill Badges Completed"', order: 'desc'}, {column: '"User Name"', order: 'asc'}]);
if (error) {
    console.log(error);
}
